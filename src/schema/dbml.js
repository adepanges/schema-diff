...
/**
 * Convert a parsed schema model (from parser.js) to a DBML string.
 */
function toDbml(schema) {
  ...
  for (const table of Object.values(schema.tables)) {
    for (const fk of table.foreignKeys) {
      const fromCols = fk.columns.length === 1 ? `${table.name}.${fk.columns[0]}` : `(${fk.columns.map((c) => `${table.name}.${c}`).join(', ')})`;
      const toCols = fk.refColumns.length === 1 ? `${fk.refTable}.${fk.refColumns[0]}` : `(${fk.refColumns.map((c) => `${fk.refTable}.${c}`).join(', ')})`;
      const attrs = [];
